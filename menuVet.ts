...
export function menuVeterinaria() {
  ...
  switch (opcion) {
    case "1":
      ...
      break;
    case "2":
      const idSeleccionado = readlineSync.questionInt(
        "Introduce el ID de la veterinaria a seleccionar: "
      );
      fs.readFile(
        "veterinarias.txt",
        "utf-8",
        (err: NodeJS.ErrnoException | null, data: string) => {
          ...
          try {
            ...
            const veterinarias: Veterinaria[] = veterinariasTxt.map(
              (vete) => new Veterinaria(vete.nombre, vete.direccion, vete.id) // Pasar el id aquí
            );

            // Buscar la veterinaria por ID
            const veterinaria = veterinarias.find(
              (v) => v.getId() === idSeleccionado
            );
...
function gestionarClientes(veterinaria: Veterinaria): void {
  ...
  switch (opcion) {
    ...
    case "2":
      ...
      const nuevoCantVisitas = readlineSync.questionInt(
        "Nueva cantidad de visitas: "
      );
